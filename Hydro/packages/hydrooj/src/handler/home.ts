...
export class HomeHandler extends Handler {
    ...
    async getRanking(domainId: string, limit = 50) {
        ...
        // 获取所有非管理员用户 (排除 PRIV_ALL 和 PRIV_EDIT_SYSTEM)
        const adminUsers = await user.getMulti({ 
            $or: [
                { priv: PRIV.PRIV_ALL },
                { priv: { $bitsAllSet: PRIV.PRIV_EDIT_SYSTEM } }
            ]
        }).project({ _id: 1 }).toArray();
        const adminUids = adminUsers.map(u => u._id);
        
        const dudocs = await domain.getMultiUserInDomain(domainId, { 
            uid: { $gt: 1, $nin: adminUids }, 
            rp: { $gt: 0 } 
