...
EOF
        ...
        # 使用温和的超时/重试，不再按速度阈值判失败，避免慢网速下误判
        CURL_OPTS=(--fail -L --retry 3 --retry-delay 3 --connect-timeout 10 --max-time 900)
...
start_services() {
    ...
    # 启动 UI 开发服务器（按当前仓库 UI 源码运行，默认端口 8044）
    if [[ -f "$ROOT_DIR/packages/ui-default/build/index.js" ]]; then
        log_info "启动 hydro-web（UI 开发服务器，默认端口 8044）..."
        pm2 start "node $ROOT_DIR/packages/ui-default/build --dev" --name hydro-web --update-env >/dev/null 2>&1 || \
        pm2 restart hydro-web --update-env >/dev/null 2>&1 || \
        log_warning "hydro-web 启动/重启失败，请检查 packages/ui-default/build 是否完整"
        pm2 save
    else
